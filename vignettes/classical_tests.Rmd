...
comparing classical hypothesis tests (Wald, LR, Score) for $\beta$ARMA model. 
...
```{r, data_simul_plot, include=FALSE}
...
Across the board, the recommended, adjusted tests 
(LR3, Sr, Se, and the Wald tests) consistently produce high p-values 
(e.g., p = 0.9898 for LR3 , p = 0.9862 for Sr and Se ), leading to the same 
conclusion: we do not reject the null hypothesis ($H_0$).
This is the correct inference, as the data was simulated from a true 
$\beta$AR(1) model with no MA components.

In contrast, the naive LR$_1$ statistic produced a negative value, which is 
nonsensical for a test statistic that should follow a $\chi^2$ distribution, 
highlighting its unreliability in this scenario.

This confirms the central finding from the source paper: using the 
methodologically correct, adjusted tests is critical for accurate inference. 
This example demonstrates the ability to not only implement these tests but 
also to select the appropriate version to ensure robust conclusions.
...
models, replicating the findings of Costa, Cribari-Neto, and Scher (2024).
 ...
 * Statistical Theory: Deep understanding of the theoretical underpinnings of 
 three core statistical tests.

 * Methodological Rigor: Identifying and correcting for subtle issues in 
 statistical procedures that can impact results.

 * R Programming: Translating complex statistical formulas from a research 
 paper into functional, reproducible R code within a package structure.

 * Validation & Simulation: Using simulated data with a known ground truth to 
