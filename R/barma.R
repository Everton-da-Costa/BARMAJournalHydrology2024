#' @title Beta Autoregressive Moving Average (\betaARMA) Model Estimation
#'
#' @description
#' Fits BARMA, BAR, BMA, BARMAX, BARX, or BMAX models for time series data
...
#'   models. \emph{TEST}, 18(3), 529–545. \doi{10.1007/s11749-008-0116-8}.
#'   (Erratum: \emph{TEST}, 26, 451–459, 2017. \doi{10.1007/s11749-016-0517-7}).
#'
#' Ferrari, S.L.P., & Cribari-Neto, F. (2004). Beta Regression for Modelling
#'   Rates and Proportions. \emph{Journal of Applied Statistics}, 31(7), 799-815.
#'   \doi{10.1080/0266476042000214501}.
#'
#' @author
#' Original R code by: Fabio M. Bayer 
#' (Federal University of Santa Maria, <bayer@ufsm.br>)
#' Modified and improved by: Everton da Costa 
#' (Federal University of Pernambuco, <everto.cost@gmail.com>)
#'
...
#' set.seed(123)
#' y_placeholder <- rbeta(100, 2, 2) * 0.8 + 0.1 # Ensure (0,1)
#' y_ts <- ts(y_placeholder)
#'
#' # Fit a BARMA(1,1) model
#' if (requireNamespace("lbfgs", quietly = TRUE)) {
#'   model1 <- barma(y = y_ts, ar = 1, ma = 1, link = "logit", h1 = 10)
#'   if (!is.null(model1$model)) print(model1$model)
#' }
#'
#' # Fit a BAR(1) model with an exogenous variable
#' X_reg <- rnorm(100)
#' X_reg_ts <- ts(X_reg)
#' X_fut_reg <- rnorm(10)
#' if (requireNamespace("lbfgs", quietly = TRUE)) {
#'   model2 <- barma(y = y_ts, ar = 1, X = X_reg_ts, X_hat = X_fut_reg, h1 = 10)
#'   if (!is.null(model2$model)) print(model2$model)
#' }
#' }
#'
